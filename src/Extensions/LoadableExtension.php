...
class LoadableExtension extends DataExtension
{
    ...
    {
        $fullpath = 'Loadable/' . str_replace('\\', '/', get_class($this->owner));
        ...
        else if (ss_theme_template_file_exists($shortpath))
        {
            return $this->owner->renderWith($shortpath);
        }

        $html = DBHTMLText::create();
        $html->setValue('<p>Template not found. Create one in /themes/' . ss_theme() . '/<strong>Loadable/' . $fullpath . '.ss</strong></p>');
        return $html;
    }
